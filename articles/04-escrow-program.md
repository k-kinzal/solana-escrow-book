...
`Initialize`は取引を初期化し、取引可能な状態にします。
...
`IsInitialized`の実装では、全てのフィールドが`0`値であるかで判別もできますが、アカウントの家賃とCU（Compute Unit）の使用量のトレードオフを考慮する必要があります。
...
エスクロープログラムで実際の処理をするプロセッサを実装します。
...
CU、堅牢性、実装コストのバランスを考えて、適切なバリデーションをしましょう。
...
- `seller_account`に署名があること：この後の関連トークンアカウントの所有者を変更する処理で署名の検証が行われるため不要
- `temp_token_account`が関連トークンアカウントであること：この後の関連トークンアカウントの所有者を変更する処理で検証が行われるため不要
...
- `token_program`がトークンプログラムのアカウントであること：この後の関連トークンアカウントの所有者を変更する処理が実行できなくなるため不要（Token Extensionのように後方互換性のある別のプログラムに切り替わる可能性もあるため、厳密な検証はできません）
